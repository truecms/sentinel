...
PATTERNS = [
    ...
    (
        r"(\s+)_=(\[.*\]),\s*# drupal_core_compatibility",
        r"\1drupal_core_compatibility=\2,",
    ),
    # Site patterns
    (r'(\s+)_=("https://.*\.example\.com"),', r"\1url=\2,"),
    # Common audit fields
    (
        r"(\s+)_=(test_user\.id|org_admin\.id|org_user\.id),\s*# created_by",
        r"\1created_by=\2,",
    ),
    (
        r"(\s+)_=(test_user\.id|org_admin\.id|org_user\.id),\s*# updated_by",
        r"\1updated_by=\2,",
    ),
    (r"(\s+)_=(True),\s*# is_active", r"\1is_active=\2,"),
