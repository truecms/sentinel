...
def upgrade() -> None:
    ...
    op.alter_column('module_versions', 'is_security_update',
               existing_type=sa.BOOLEAN(),
               nullable=False)
    op.alter_column('module_versions', 'release_notes',
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(op.f('sites_site_uuid_key'), 'sites', ['site_uuid'], postgresql_nulls_not_distinct=False)
    op.create_index(op.f('ix_sites_security_score'), 'sites', ['security_score'], unique=False)
    op.create_index(op.f('ix_sites_last_drupal_org_check'), 'sites', ['last_drupal_org_check'], unique=False)
    op.create_index(op.f('ix_sites_last_data_push'), 'sites', ['last_data_push'], unique=False)
    op.drop_constraint(None, 'organizations', type_='foreignkey')
    op.drop_constraint(None, 'organizations', type_='foreignkey')
    op.create_unique_constraint(op.f('modules_machine_name_key'), 'modules', ['machine_name'], postgresql_nulls_not_distinct=False)
    op.alter_column('module_versions', 'release_notes',
