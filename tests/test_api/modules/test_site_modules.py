...
class TestSiteModulesList:
    ...
    async def test_get_site_modules_success(
        ...
        response = await client.get(
            f"/api/v1/sites/{org_test_site.id}/modules", headers=org_user_token_headers
        )
    ...
    async def test_get_site_modules_not_found(
        self, client: AsyncClient, org_user_token_headers: dict
    ):
        """Test getting modules for non-existent site."""
        response = await client.get(
            "/api/v1/sites/99999/modules", headers=org_user_token_headers
        )
    ...
    async def test_get_site_modules_org_permission(
        ...
        other_org = Organization(name="Other Org", created_by=org_admin.id)
        ...
        response = await client.get(
            f"/api/v1/sites/{other_site.id}/modules", headers=headers
        )
        assert response.status_code == 403

    async def test_get_site_modules_requires_auth(
        self, client: AsyncClient, org_test_site: Site
    ):
...
class TestSiteModuleCreate:
    ...
    async def test_add_site_module_requires_auth(
        ...
        response = await client.post(
            f"/api/v1/sites/{org_test_site.id}/modules", json=site_module_data
        )
        assert response.status_code == 401
...
class TestSiteModuleUpdate:
    ...
    async def test_update_site_module_not_found(
        self, client: AsyncClient, org_test_site: Site, org_user_token_headers: dict
    ):
    ...
    async def test_update_site_module_requires_auth(
        self, client: AsyncClient, org_test_site: Site, org_test_site_module: SiteModule
    ):
...
class TestSiteModuleStats:
    ...
    async def test_get_site_module_stats_not_found(
        ...
        response = await client.get(
            "/api/v1/sites/99999/modules/stats", headers=org_user_token_headers
        )
        assert response.status_code == 404
...
class TestModuleSites:
    ...
    async def test_get_module_sites_not_found(
        ...
        response = await client.get(
            "/api/v1/modules/99999/sites", headers=org_user_token_headers
        )
        assert response.status_code == 404

    async def test_get_module_sites_requires_auth(
        self, client: AsyncClient, org_test_module: Module
    ):
