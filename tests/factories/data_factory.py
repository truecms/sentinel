...
class TestDataFactory:
    ...
    def _load_templates(self):
        ...
        base_path = Path(__file__).parent.parent.parent / "examples" / "drupal-submissions"
        
        # Load existing templates
        self.minimal_modules = self._load_json_template(base_path / "minimal-site.json")
        self.standard_modules = self._load_json_template(base_path / "standard-site.json") 
        ...
        self.core_modules = [
            ...
            {"machine_name": "taxonomy", "display_name": "Taxonomy", "module_type": "core"},
            {"machine_name": "file", "display_name": "File", "module_type": "core"},
        ]
        
        # Popular contrib modules
        self.contrib_modules = [
            {"machine_name": "webform", "display_name": "Webform", "module_type": "contrib"},
            {"machine_name": "paragraphs", "display_name": "Paragraphs", "module_type": "contrib"},
            {"machine_name": "pathauto", "display_name": "Pathauto", "module_type": "contrib"},
            {"machine_name": "token", "display_name": "Token", "module_type": "contrib"},
            {"machine_name": "admin_toolbar", "display_name": "Admin Toolbar", "module_type": "contrib"},
            {"machine_name": "metatag", "display_name": "Metatag", "module_type": "contrib"},
            {"machine_name": "entity_reference_revisions", "display_name": "Entity Reference Revisions", "module_type": "contrib"},
            {"machine_name": "redirect", "display_name": "Redirect", "module_type": "contrib"},
            {"machine_name": "google_analytics", "display_name": "Google Analytics", "module_type": "contrib"},
            {"machine_name": "backup_migrate", "display_name": "Backup and Migrate", "module_type": "contrib"},
        ]
        
        # Custom module templates
        self.custom_modules = [
            {"machine_name": "site_theme", "display_name": "Site Theme", "module_type": "custom"},
            {"machine_name": "api_integration", "display_name": "API Integration", "module_type": "custom"},
            {"machine_name": "custom_blocks", "display_name": "Custom Blocks", "module_type": "custom"},
            {"machine_name": "site_config", "display_name": "Site Configuration", "module_type": "custom"},
            {"machine_name": "data_import", "display_name": "Data Import Tool", "module_type": "custom"},
        ]
    ...
    async def create_large_enterprise_site(
        ...
        additional_contrib = [
            {"machine_name": f"contrib_module_{i}", "display_name": f"Contrib Module {i}", "module_type": "contrib"}
            for i in range(1, 51)  # 50 additional contrib modules
        ]
        
        # Generate additional custom modules for enterprise site
        additional_custom = [
            {"machine_name": f"custom_module_{i}", "display_name": f"Custom Module {i}", "module_type": "custom"}
    ...
    async def create_bulk_test_data(
        ...
        return {
            ...
            "site_data": [
                {
                    "site": site,
                    "module_count": len([sm for sm in all_site_modules if sm.site_id == site.id])
                }
    ...
    async def _create_module_version(self, module: Module, user: User) -> ModuleVersion:
        ...
        if module.module_type == "core":
            ...
            version_string = f"{random.randint(1, 3)}.{random.randint(0, 10)}.{random.randint(0, 5)}"
...
async def create_populated_database(
    ...
    return {
        ...
        "total_modules": len(set(
            m.id for m in (standard_site["modules"] + minimal_site["modules"] + security_data["security_modules"])
        ))
    }
