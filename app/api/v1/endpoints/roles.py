...
async def list_roles(
    ...
    auth_subject: Union[User, Site] = Depends(require_resource_action_dependency("user", "read"))
) -> List[Role]:
...
async def assign_role_to_user(
    ...
    request: Request = None,
    db: AsyncSession = Depends(deps.get_db),
    auth_subject: Union[User, Site] = Depends(require_resource_action_dependency("user", "assign_roles"))
) -> UserRole:
...
async def revoke_role_from_user(
    ...
    if not user_role.is_active():
        raise HTTPException(status_code=400, detail="Role assignment is already inactive")
...
async def check_user_permission(
    ...
    if "permission" in permission_data:
        has_permission = user.has_permission(permission_data["permission"], organization_id)
