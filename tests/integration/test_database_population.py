...
class TestDatabasePopulation:
    ...
    async def test_minimal_drupal_site_creation(self, minimal_drupal_site: dict):
        ...
        expected_core = {"views", "node", "user", "system", "field", "text", "taxonomy", "file"}
    ...
    async def test_bulk_test_data_creation(self, bulk_test_data: dict):
        ...
        for site_info in site_data:
            ...
            assert site_info["module_count"] >= 15  # Each site should have reasonable modules
...
class TestSampleDataConsistency:
    ...
    async def test_sample_drupal_modules_structure(self, sample_drupal_modules: list[dict]):
        ...
        for module_data in sample_drupal_modules:
            ...
            assert "_" in module_data["machine_name"] or module_data["machine_name"].isalpha()
    ...
    async def test_security_update_modules_structure(self, security_update_modules: list[dict]):
        """Test security update modules data structure."""
        assert len(security_update_modules) == 3  # Should have exactly 3 security scenarios
