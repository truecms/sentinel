...
class UpdateDetector:
    ...
    async def check_module_updates(
        ...
        security_available, security_version = self.comparator.is_security_update_available(
            current_version,
            compatible_versions
        )
    ...
    async def check_site_module_updates(
        ...
        result = await db.execute(
            select(SiteModule)
            .where(SiteModule.id == site_module_id)
        )
    ...
    async def batch_check_updates(
        self,
        db: AsyncSession,
        module_versions: List[Tuple[int, str]]
    ) -> Dict[int, UpdateInfo]:
        """
    ...
    async def get_modules_needing_check(
        ...
        stmt = (
            ...
            .where(
                and_(
                    SiteModule.current_version.isnot(None),
