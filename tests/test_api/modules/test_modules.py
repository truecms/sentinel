...
class TestModulesList:
    ...
    async def test_get_modules_success(
        ...
        response = await client.get("/api/v1/modules", headers=user_token_headers)
    ...
    async def test_get_modules_pagination(
        ...
        response = await client.get(
            "/api/v1/modules?skip=0&limit=3", headers=user_token_headers
        )
        assert response.status_code == 200
        ...
        # Test second page
        response = await client.get(
            "/api/v1/modules?skip=3&limit=3", headers=user_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_get_modules_search(
        ...
        response = await client.get(
            "/api/v1/modules?search=admin_toolbar", headers=user_token_headers
        )
        assert response.status_code == 200
        ...
        response = await client.get(
            "/api/v1/modules?search=Chaos", headers=user_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_get_modules_filter_by_type(
        ...
        response = await client.get(
            "/api/v1/modules?module_type=contrib", headers=user_token_headers
        )
        assert response.status_code == 200
        ...
        response = await client.get(
            "/api/v1/modules?module_type=custom", headers=user_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_get_modules_invalid_filters(
        ...
        # Invalid module type
        response = await client.get(
            "/api/v1/modules?module_type=invalid", headers=user_token_headers
        )
        assert response.status_code == 422

        # Invalid sort field
        response = await client.get(
            "/api/v1/modules?sort_by=invalid_field", headers=user_token_headers
        )
        assert response.status_code == 422

        # Invalid sort order
        response = await client.get(
            "/api/v1/modules?sort_order=invalid", headers=user_token_headers
        )
        assert response.status_code == 422
...
class TestModuleCreate:
    ...
    async def test_create_module_success(
        ...
        response = await client.post(
            "/api/v1/modules", json=module_data, headers=superuser_token_headers
        )
    ...
    async def test_create_module_requires_superuser(
        ...
        response = await client.post(
            "/api/v1/modules", json=module_data, headers=user_token_headers
        )
    ...
    async def test_create_module_validation_errors(
        ...
        response = await client.post(
            "/api/v1/modules", json={}, headers=superuser_token_headers
        )
        assert response.status_code == 422
    ...
    async def test_create_module_requires_auth(self, client: AsyncClient):
        ...
        response = await client.post("/api/v1/modules", json=module_data)
        assert response.status_code == 401
...
class TestModuleDetail:
    ...
    async def test_get_module_success(
        ...
        response = await client.get(
            f"/api/v1/modules/{test_module_id}", headers=user_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_get_module_not_found(
        ...
        response = await client.get("/api/v1/modules/99999", headers=user_token_headers)
...
class TestModuleUpdate:
    ...
    async def test_update_module_not_found(
        ...
        response = await client.put(
            "/api/v1/modules/99999", json=update_data, headers=superuser_token_headers
        )
        assert response.status_code == 404
    ...
    async def test_update_module_requires_auth(
        ...
        response = await client.put(
            f"/api/v1/modules/{test_module.id}", json=update_data
        )
        assert response.status_code == 401
...
class TestModuleDelete:
    ...
    async def test_delete_module_success(
        ...
        response = await client.delete(
            f"/api/v1/modules/{test_module_id}", headers=superuser_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_delete_module_not_found(
        ...
        response = await client.delete(
            "/api/v1/modules/99999", headers=superuser_token_headers
        )
        assert response.status_code == 404
    ...
    async def test_delete_module_requires_superuser(
        ...
        response = await client.delete(
            f"/api/v1/modules/{test_module.id}", headers=user_token_headers
        )
        assert response.status_code == 403
...
class TestModuleBulkOperations:
    ...
    async def test_bulk_create_modules_success(
        ...
        response = await client.post(
            "/api/v1/modules/bulk", json=modules_data, headers=superuser_token_headers
        )
        assert response.status_code == 200
    ...
    async def test_bulk_modules_limit_exceeded(
        ...
        modules_data = [
            {"machine_name": f"bulk_module_{i}", "display_name": f"Bulk Module {i}"}
    ...
    async def test_bulk_modules_requires_superuser(
        ...
        modules_data = [
            {"machine_name": "unauthorized_bulk", "display_name": "Unauthorized Bulk"}
        ]

        response = await client.post(
            "/api/v1/modules/bulk", json=modules_data, headers=user_token_headers
        )
        assert response.status_code == 403
    ...
    async def test_bulk_modules_requires_auth(self, client: AsyncClient):
        ...
        modules_data = [
            {"machine_name": "no_auth_bulk", "display_name": "No Auth Bulk"}
        ]

        response = await client.post("/api/v1/modules/bulk", json=modules_data)
        assert response.status_code == 401
