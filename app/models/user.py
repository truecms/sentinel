...
class User(Base):
    ...
    assigned_roles = relationship("UserRole", back_populates="assigned_by", foreign_keys="[UserRole.assigned_by_id]")
    ...
    def has_permission(self, permission_name: str, organization_id: Optional[int] = None) -> bool:
        """Check if user has a specific permission."""
        roles = self.get_roles(organization_id)
        return any(role.has_permission(permission_name) for role in roles)

    def has_resource_action(self, resource: str, action: str, organization_id: Optional[int] = None) -> bool:
