...
class TestSitesOverview:
    ...
    async def test_get_sites_overview_success(
        ...
        response = await client.get("/api/v1/sites/overview", headers=user_token_headers)
        ...
        site = sites[0]
        required_fields = [
            "id", "name", "url", "security_score", "total_modules_count",
            "security_updates_count", "non_security_updates_count",
            "last_data_push", "last_drupal_org_check", "status", "organization_id"
        ]
    ...
    async def test_sites_overview_pagination(
        ...
        # Test with limit
        response = await client.get("/api/v1/sites/overview?limit=2", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
        ...
        # Test with skip
        response = await client.get("/api/v1/sites/overview?skip=1&limit=2", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_sites_overview_search(
        ...
        # Search by site name
        response = await client.get("/api/v1/sites/overview?search=Healthy", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
        ...
        # Search by URL
        response = await client.get("/api/v1/sites/overview?search=warning.example", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get("/api/v1/sites/overview?search=CRITICAL", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_sites_overview_sorting(
        ...
        response = await client.get("/api/v1/sites/overview?sort_by=security_score&sort_order=asc", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
        ...
        response = await client.get("/api/v1/sites/overview?sort_by=security_score&sort_order=desc", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
        ...
        # Sort by name
        response = await client.get("/api/v1/sites/overview?sort_by=name&sort_order=asc", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_sites_overview_empty_result(
        ...
        response = await client.get("/api/v1/sites/overview?search=nonexistent", headers=user_token_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_sites_overview_invalid_sort_order(
        ...
        response = await client.get("/api/v1/sites/overview?sort_order=invalid", headers=user_token_headers)
