...
async def create_module(
    db: AsyncSession, module: ModuleCreate, created_by: int
) -> Module:
    """Create a new module."""
    db_module = Module(
...
async def update_module(
    db: AsyncSession, module_id: int, module_update: ModuleUpdate, updated_by: int
) -> Optional[Module]:
...
async def delete_module(
    db: AsyncSession, module_id: int, updated_by: int
) -> Optional[Module]:
...
async def bulk_upsert_modules(
    db: AsyncSession, modules: List[Dict[str, Any]], created_by: int
) -> Dict[str, Any]:
        ...
        try:
            ...
            if not machine_name:
                ...
                results["errors"].append(
                    {"module": module_data, "error": "machine_name is required"}
                )
                continue
            ...
            if existing_module:
                ...
                update_data = ModuleUpdate(
                    **{k: v for k, v in module_data.items() if k != "machine_name"}
                )
            ...
            results["failed"] += 1
            results["errors"].append({"module": module_data, "error": str(e)})
