...
async def create_site(
    *,
    ...
    if site:
        raise HTTPException(
            status_code=409, detail="The site with this URL already exists."
        )
...
async def get_sites_overview(
    ...
    for site in sites_data:
        ...
        if site.security_updates_count > 0:
            ...
            site.security_score < settings.SECURITY_SCORE_CRITICAL_THRESHOLD
            or site.non_security_updates_count
            > settings.NON_SECURITY_UPDATES_WARNING_THRESHOLD
        ):
            status = "warning"
    ...
    pagination = {"page": page, "per_page": limit, "total": total, "total_pages": pages}

    # Build response filters
    response_filters = {"search": search, "sort_by": sort_by, "sort_order": sort_order}
...
async def get_site_modules(
    ...
    updates_only: bool = Query(
        False, description="Show only modules with available updates"
    ),
...
async def sync_site_modules(
    ...
    *,
    ...
    auth_subject: Union[User, Site] = Depends(
        require_resource_action_dependency("site", "sync")
    ),
    request: Request,
):
    """
    ...
    if isinstance(auth_subject, User):
        ...
        if (
            not auth_subject.is_superuser
            and auth_subject.organization_id != site.organization_id
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
    ...
    if len(payload.modules) > 500:
        ...
        task = sync_site_modules_task.delay(
            site_id=site_id, payload_json=payload_json, user_id=current_user_id
        )
    ...
    for module_info in payload.modules:
        try:
            ...
            if not module:
                ...
                # Invalidate cache
                await ModuleCacheService.invalidate_module_cache(
                    module_info.machine_name
                )
            ...
            if site_module and module_info.enabled:
                try:
                    ...
                    if (
                        ...
                        or update_info.security_update_available
                    ):
            ...
            errors.append({"module": module_info.machine_name, "error": str(e)})
