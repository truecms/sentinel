...
from app.api.deps import get_db, get_current_user_or_site
...
class PermissionChecker:
    ...
    async def check_permission(
        ...
        if isinstance(auth_subject, User):
            return await self.user_has_permission(auth_subject, permission, organization_id)
    ...
    async def check_resource_action(
        ...
        if isinstance(auth_subject, User):
            return await self.user_has_resource_action(auth_subject, resource, action, organization_id)
        elif isinstance(auth_subject, Site):
